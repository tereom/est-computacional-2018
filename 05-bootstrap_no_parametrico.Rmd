...
Estas notas se desarrollaron cocn base en @efron y abordan los siguientes temas:
...
una distribución normal, el bootstrap se puede usar para estimar 
...
$$(\hat{\theta}- z^{(1-\alpha)}\cdot \hat{se}, \hat{\theta}-z^{(\alpha)}\cdot \hat{se})$$ este intervalo no es preciso cuando $\hat{\theta}$ no se distribuye Normal.
