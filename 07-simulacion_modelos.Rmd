...
decatcausa <- read_csv("data/decatcausa.csv")
deedad <- read_csv("data/deedad.csv")
...
calcular_tasas <- function(causa){
    ...
    list(defun_mun = defun_mun, defun_entidad = defun_entidad)
}
tasas_diabetes <- calcular_tasas("diabetes")
write_csv(tasas_diabetes$defun_entidad, "data/tasas_diabetes.csv")
```

```{r echo = FALSE, warning=FALSE,cache = TRUE, message = FALSE, eval=FALSE}
library(sf)
library(nullabor)

tasas_diabetes <- read.csv("data/tasas_diabetes.csv")
set.seed(74872)
nulos <- lineup(null_permute("tasa_causa"), tasas_diabetes, n = 6)
edo_shp <- st_read("data/shapes/Mex_Edos.shp", stringsAsFactors = FALSE, 
    quiet = TRUE)
nulos_map <- edo_shp %>% 
    mutate(ENT = ifelse(NOM_ENT == "Coahuila de Zaragoza", "Coahuila", 
        NOM_ENT)) %>% 
    left_join(nulos, by = c("ENT" = "ent"))

ggplot() + 
    geom_sf(data = filter(nulos_map, .sample == 2), aes(fill = tasa_causa), 
        size = 0.2) +
...
ggsave("imagenes/tasas_cancer_1.png")
```
...
```{r lineup_diabetes, echo = FALSE, out.width="800px", eval=FALSE}
ggplot() + 
    geom_sf(data = nulos_map, aes(fill = tasa_causa), 
        size = 0.2) +
...
```{r, fig.width=4.5, fig.height=3.8, out.width="350px", cache = TRUE}
...
```{r, fig.height=3, fig.width=3.3, out.width="350px"}
...
```{r, fig.height=3, fig.width=3.3, out.width="350px",warning=FALSE}
